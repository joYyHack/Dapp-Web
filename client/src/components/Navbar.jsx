...
const Navbar = () => {
  const [isToggleMenuOn, setToggleMenu] = React.useState(false);
  return (
    <nav className="w-full p-4 flex md:justify-center items-center justify-between ">
      <ul className="text-white list-none hidden md:flex flex-row justify-between items-center flex-initial">
        {["Marker", "Exchange", "Tutorials", "Wallets"].map(
          (itemName, index) => (
            <NavbarItem key={itemName + index} title={itemName} />
          )
        )}
        <li className="bg-cyan-500 hover:bg-cyan-900 ease-linear duration-200 mx-4 px-7 py-2 text-center rounded-full cursor-pointer ">
          Login
        </li>
      </ul>
